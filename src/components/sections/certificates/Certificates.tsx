...
const CertificateCard = ({
  ...
  return (
    <a
      ...
      className="group flex-shrink-0 w-[350px] bg-[#0f0f0f] rounded-xl border border-[#1c1c1c] hover:border-[#2a2a2a] overflow-hidden transition-all duration-300 hover:scale-[1.02]"
    >
      {/* Certificate Image */}
        ...
        {tags && tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {tags.slice(0, 3).map((tag: string, index: number) => (
              <span
                key={index}
                className="px-2.5 py-1 bg-white/5 text-white/60 rounded-md text-xs font-medium border border-[#1c1c1c]"
              >
                {tag}
              </span>
            ))}
            {tags.length > 3 && (
              <span className="px-2.5 py-1 text-[#777] text-xs">
                +{tags.length - 3} more
              </span>
            )}
          </div>
